...
<body>
    ...
    <?php
        ...
        for ($i = 0; $i < count($adatTomb); $i++) {
            if (substr($adatTomb[$i][2], 0, 7) == '1999-10') {
                ...
                $kiir1 .= '<td style="text-align:right;">' . round((int)$adatTomb[$i][4] * 2.54, 1) . ' cm</td>';
                $kiir1 .= '</tr>'; 
            }
        }
        $kiir1 .= '</table>';
    ...
    <form method='post' action='<?php echo $_SERVER["PHP_SELF"]?>'>
        <span>Kérek egy 1990 és 1999 közötti évszámot: </span>
    ...
    <?php
        ...
        for ($i = 0; $i < count($adatTomb); $i++) {
            $kezdo = (int)substr($adatTomb[$i][1], 0, 4);
            $veg = (int)substr($adatTomb[$i][2], 0, 4);
            if ($kezdo <= $esztendo && $esztendo <= $veg) {
